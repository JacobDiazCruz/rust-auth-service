...
//     let response = logout_user_service(State(app_state), headers).await;
//     match response {
//         Ok(data) => Ok(data),
//         Err(err) => Err(err),
//     }
...
// pub async fn refresh_token_api(req: axum::extract::RequestParts<impl Send>) -> impl IntoResponse {
